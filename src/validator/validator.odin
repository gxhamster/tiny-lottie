...
parse_dependent_schemas :: proc(
  ...
  if properties_as_object, ok := value.(json.Object); ok {
    ...
    // with and handle.
    schema.dependent_schemas = make_slice(
      []PoolIndex,
      no_of_properties,
      allocator,
    )
    ...
    if no_of_properties > 0 {
      ...
      for prop_field in properties_as_object {
        ...
        if err != .None {
          log.infof(
            "parsing a dependent schema failed (field=%v) : %v",
            prop_field,
            err,
          )
          return err
...
parse_pattern_properties :: proc(
  ...
  if properties_as_object, ok := value.(json.Object); ok {
    ...
    no_of_properties := len(properties_as_object)
    schema.pattern_regex = make(
      [dynamic]regex.Regular_Expression,
      0,
      allocator,
    )
    ...
    for prop_field in properties_as_object {
      ...
      if err != .None {
        log.infof(
          "Parsing prefix properties failed (field=%v) : %v",
          prop_field,
          err,
        )
        return err
      ...
      case regex_parser.Error:
        log.infof(
          "Regex parser error (%v)",
          regex_create_err.(regex_parser.Error),
        )
...
parse_allof :: proc(
  ...
  if value_as_array, ok := value.(json.Array); ok {
    ...
    for subschema_val in value_as_array {
      _, idx, _ := parse_schema_from_json_value(
        subschema_val,
        schema_context,
        allocator,
      )
...
parse_schema_from_string :: proc(
  ...
  parsed_json, parsed_json_err := json.parse_string(
    schema,
...
validate_properties :: proc(
  ...
  for prop in subschema.properties_children {
    ...
    if val != nil {
      prop_valid_err := validate_json_value_with_subschema(
        val,
        prop_schema,
        ctx,
      )
...
validate_dependent_schemas :: proc(
  json_value: json.Value,
  subschema: ^Schema,
  ctx: ^Context,
) -> Error {
  ...
  for prop_idx in subschema.dependent_schemas {
    ...
    assert(
      prop_schema.name != "",
      "Name not defined for dependentSchema property",
    )
    dependent_prop_exists := prop_schema.name in json_value_as_obj
    if dependent_prop_exists {
      dependent_schema_err := validate_json_value_with_subschema(
        json_value_as_obj,
        prop_schema,
        ctx,
      )
      if dependent_schema_err != .None {
        log.infof(
          "dependentSchemas validation failed (%v)",
          dependent_schema_err,
        )
...
validate_additional_properties :: proc(
  ...
  for obj_prop in json_value_as_obj {
    ...
    if !matches_properties && !matches_pattern_properties {
      val := json_value_as_obj[obj_prop]
      additional_properties_schema := get_schema(
        ctx,
        subschema.additional_properties,
      )
      additional_prop_err := validate_json_value_with_subschema(
        val,
      ...
      if additional_prop_err != .None {
        log.infof(
          "additional_properties validation failed (%v)",
          additional_prop_err,
        )
...
validate_property_names :: proc(
  ...
  for prop_name in json_value_as_obj {
    ...
    if prop_name_err != .None {
      log.infof("propertyNames validation failed (%v)", prop_name_err)
...
validate_anyof :: proc(
  ...
  for subschema_idx in subschema.anyof {
    subschema := get_schema(ctx, subschema_idx)
    if err := validate_json_value_with_subschema(json_value, subschema, ctx);
       err == .None {
...
validate_if_then_else :: proc(
  ...
  if then_exists {
    ...
    then_err := validate_json_value_with_subschema(
      json_value,
      then_schema,
      ctx,
    )
  ...
  if else_exists {
    ...
    else_err := validate_json_value_with_subschema(
      json_value,
      else_schema,
      ctx,
    )
...
validate_not :: proc(
  ...
  validation_err := validate_json_value_with_subschema(
    json_value,
    not_schema,
    ctx,
  )
...
validate_contains :: proc(
  ...
  min_contains_defined :=
    SchemaKeywords.MinContains in subschema.validation_flags
  max_contains_defined :=
    SchemaKeywords.MaxContains in subschema.validation_flags
    ...
    if SchemaKeywords.PrefixItems in subschema.validation_flags {
      for elem, idx in json_value_array {
        if idx < len(subschema.prefix_items) {
          prefix_elem_schema := get_schema(
            ctx,
            PoolIndex(subschema.prefix_items[idx]),
          )
          rest_start_idx = idx
