...
run_test_in_spec_file :: proc(
	...
	end_idx := end_test_idx != -1 ? end_test_idx : len(parsed_json.(json.Array)) - 1

	if start_idx < 0 || start_idx > end_idx || start_idx > len(parsed_json.(json.Array)) - 1 {
		log.fatalf("Invalid start test idx")
		return
	}

	if end_idx < 0 || end_idx < start_idx || end_idx > len(parsed_json.(json.Array)) - 1 {
